...
from utilities import ShapelyPolygon2Polygon, rgb2hex, polygon_intersection
...
class Visualizer:
    ...
    def _update(self, time_step, scenarios, sensor_views, ego_id, ext_sensor_view=None, ext_sensor_pos=None):
        plt.clf()
        ...
        self.plot(scenario=scenario,
                  ...
                  ext_sensor_view=ext_sensor_view,
                  ext_sensor_pos=ext_sensor_pos)
        plt.autoscale()
        plt.axis('equal')
        plt.xlim(0, 100)
        plt.ylim(-50, 50)
    ...
    def save_snapshot_plot(self, scenario, sensor_view, ego_id, file_name='figs/snapshot.png', xylimits=[40,100,-20, 40], xticks=None, yticks=None, fig_width=6, ext_sensor_view=None, ext_sensor_pos=None):
    ...
    def plot(self,
        ...
        if ext_sensor_view is not None:
            draw_params = ShapeParams.load(file_path="src/draw_params/ext_sensor_view.yaml", validate_types=False)
            ShapelyPolygon2Polygon(ext_sensor_view).draw(rnd, draw_params=draw_params)

        if ext_sensor_pos is not None:
            draw_params = ShapeParams.load(file_path="src/draw_params/ext_sensor.yaml", validate_types=False)
            Circle(0.5, ext_sensor_pos).draw(rnd, draw_params=draw_params)
        ...
        if scenario is not None:
            ...
            # Plot ext_shadows dilations due to time delays
            draw_params = ShapeParams.load(file_path="src/draw_params/ext_shadow_dilated.yaml", validate_types=False)
            if ext_sensor_view is not None:
                for shadow_obstacle in shadow_obstacles:
                    intersections = polygon_intersection(ext_sensor_view, shadow_obstacle.obstacle_shape._shapely_polygon)
                    if not intersections:
                        ...
                        for intersection in intersections:
                            ...
                            ShapelyPolygon2Polygon(intersection).draw(rnd, draw_params=draw_params)
        rnd.render()
