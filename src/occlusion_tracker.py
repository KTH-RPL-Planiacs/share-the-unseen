...
class Occlusion_tracker:
    ...
    def update_ext(self, ext_sensor_view, ext_time):
        assert(ext_time<=self.time_step)
        # TAKING INTO ACCOUNT THE TIME DELAY
        # 1. Calculate the current shadows from the external sensor
        ext_shadows = []
        for lane in self.lanes:
            lanelet_shapely = Lanelet2ShapelyPolygon(lane)
            shadow_polygons = polygon_diff(lanelet_shapely, ext_sensor_view)
            for shadow_polygon in shadow_polygons:
                if shadow_polygon.area >= self.min_shadow_area:
                    ext_shadows.append(Shadow(shadow_polygon, lane))

        # 2. Expand the external shadows based on the time delay
        time_diff = self.time_step - ext_time;
        for ext_shadow in ext_shadows:
            ext_shadow.expand(self.dt*self.max_vel*time_diff)

        # 3. Intersect the shadows if they are on the same lane
        new_shadows = []
        for shadow in self.shadows:
            for ext_shadow in ext_shadows:
                if shadow.lane.lanelet_id == ext_shadow.lane.lanelet_id:
                    intersections = polygon_intersection(shadow.polygon, ext_shadow.polygon)
