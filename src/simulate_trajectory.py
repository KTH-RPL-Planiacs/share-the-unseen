...
def step_simulation(scenario, configuration):
    ...
    percieved_scenarios = []
    sensor_views = []
    bool_ext_sensor = configuration.get('ext_sensor')
    delay = configuration.get('ext_sensor_delay')
    ext_percieved_scenarios = []
    ext_sensor_views = []
    ...
    if bool_ext_sensor==True:
        ext_sensor = Sensor(np.array([configuration.get('ext_sensor_x'),
                                      configuration.get('ext_sensor_y')]),
                        field_of_view=configuration.get(
                            'ext_field_of_view_degrees')*2*np.pi/360,
                        min_resolution=configuration.get('ext_min_resolution'),
                        view_range=configuration.get('ext_view_range'))
    ...
    for step in range(simulation_steps+1):
        ...
        ext_percieved_scenario = None
        if bool_ext_sensor==True:
            ext_percieved_scenario = copy.deepcopy(scenario)
            for obstacle in ext_percieved_scenario.obstacles:
                ext_percieved_scenario.remove_obstacle(obstacle)
        ...
        ext_sensor_view = None
        if bool_ext_sensor==True:
            # ext_sensor.update(ego_vehicle.initial_state) # In case we want the external sensor to move
            ext_sensor_view = ext_sensor.get_sensor_view(scenario)
            ext_observed_obstacles, _ = ext_sensor.get_observed_obstacles(ext_sensor_view, scenario)
            ext_percieved_scenario.add_objects(ext_observed_obstacles)

        sensor_views.append(sensor_view)
        ext_sensor_views.append(ext_sensor_view)
        ...
        if bool_ext_sensor==True and step >= delay:
            occ_track.update_ext(ext_sensor_views[-(delay+1)], ego_vehicle.initial_state.time_step-delay)
        shadow_obstacles = occ_track.get_dynamic_obstacles(percieved_scenario)
        percieved_scenario.add_objects(shadow_obstacles)
        
        if bool_ext_sensor==True:
            for observed_obstacle in ext_observed_obstacles:
                try:
                    percieved_scenario.add_objects(observed_obstacle)
                except:
                    pass
        ...
        ext_percieved_scenarios.append(ext_percieved_scenario)
    ...
    return ego_vehicle, percieved_scenarios, sensor_views, ext_percieved_scenarios, ext_sensor_views
